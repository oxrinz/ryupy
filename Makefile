NVCC = nvcc
CXXFLAGS = -Xcompiler -fPIC -std=c++17  # Added -std=c++17 here
PYTHON = python3
...
$(TARGET): $(OBJS)
	@mkdir -p $(dir $@)
	$(NVCC) -shared -std=c++17 -arch=sm_75 -DCUBLAS_ENABLED -lcublas -lcudnn -lcurand -lcudadevrt -lcudart_static \
		-L$(CUDA_STATIC_LIB_DIR) $(OBJS) $(PYBIND_INCLUDES) -I$(PYTHON_INCLUDE) \
		-L$(PYTHON_LIB_DIR) -lpython3.10 -o $(TARGET)
